...
| [02 — I ran adversarial audits against my own benchmarks, and every impressive number fell](02-auditing-my-own-benchmarks.md) | A systematic adversarial audit walked the RAM headline from 124x to ~40x to ~72x as comparisons were made fair, retracted a throughput "win" to an honest tie, recorded a performance gate as FAILED before re-scoping it, and an independent auditor found a live contract-violation bug (librdkafka infinite-retry loop) that the self-audit had missed entirely. |
