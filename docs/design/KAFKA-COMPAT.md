...
| `Produce` | 0 | v0–v7 | parses the request envelope + the records blob |
...
record values stream through a `KafkaChannelSource` into the normal rail (`board` → **seal** → substrate → `offload`
→ `Sink`). Produced offsets are tracked per `(topic, partition)` and returned in the Produce response.

**Verified:** `kafka-ingest.yml` (CI) runs a real `kcat`/librdkafka producer → datarail → sealed rail → a Postgres
service container, asserting the sealed rows land. Continuously gated, not a one-off (`LASTRO-MATRIX`).
...
- **No idempotent / transactional producer** (`InitProducerId`, the transaction APIs). Delivery is at-least-once;
  a producer with `enable.idempotence=true` that requires `InitProducerId` is not yet supported.
...
4. **Idempotent producer** (`InitProducerId`) → exactly-once from idempotent producers.
