...
| AC-10 fairness benchmark | **PARTIAL** — real Kafka measured; honest verdict, not rigged | Rig built (`datarail-bench::fairness`, byte-equal anti-cheat). **Real Apache Kafka 3.8 (KRaft) run on the VAES cloud** (2 vCPU, capped heap): **90,661 rec/s = 22.1 MB/s** (256 B records, acks=1, **plaintext**, p99 949 ms). datarail on the same VAES core class: **sealed AEAD 1.43 GB/s (default) / 5.92 GB/s (vaes)** batched; per-1-record-cofre ~118 µs (~8.5 k/s). **Honest verdict (no rigging):** (1) **bulk + sealed** — datarail's design mode crushes Kafka (GB/s sealed vs tens of MB/s plaintext) **and** is provider-blind, which Kafka structurally cannot be; (2) **unbatched tiny records** — datarail's per-cofre crypto makes it *slower* than Kafka's lighter per-record path (~8.5 k/s vs ~90 k/s) → which is exactly why the SPEC mandates batch-many-records-per-cofre (amortizes the wrap, then it dominates). The **perfectly matched byte-equal bake-off** (identical records+batching through both via the rig) is the remaining refinement; the directional + structural verdict is clear and documented honestly. |
