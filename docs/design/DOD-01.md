...
> (needs absent resources). **16 crates · 133 tests** · clippy `deny(all+pedantic)` clean. `forbid(unsafe)`
...
| AC-3 forge-reject (differential) | **PROVEN** | `datarail-cofre::ac3_wrong_and_forged_key_rejected` + `datarail-terminal::ac9_forged_cofre_wrong_signer`. **Identity layer (F2/F3) built + reachable** (`datarail-identity`, exercised by `datarail pair`): `Noise_KK` mutual-static auth (impostor pin ⇒ handshake fails) + SPAKE2 short-code bootstrap (cold-verified) — the channel-auth counterpart to the per-cofre seal. (The live two-process pairing / Noise-protected-substrate-hop is the documented next integration; the primitives + a local end-to-end rehearsal are done.) |
...
1. **GATE-WARP — representative VAES hardware** (#13): the pass measurement needs real x86 VAES HW (this box is Rosetta-x86_64, no VAES). Setting the design-target `X` + the CI-gate policy is a tech-lead call; the representative *measurement* is hardware-gated. The guard fails until X is set on the target (MAJ-7).
...
Performance is **DIRECTIONAL** (this box is Rosetta-x86_64, not the VAES target). Nothing is faked.
