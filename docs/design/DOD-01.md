...
| AC-8 WAN/resume | **PROVEN** (real-socket kill-resume + WAN harness) / DIRECTIONAL bench; `bao` PENDING | (a) **Real-socket kill-resume PROVEN**: `real_socket_resume.rs` partitions a **real TCP** connection mid-stream, reconnects, re-drives the outbox, and the effectively-once gate yields **0-loss/0-dup** in order (upgrades the in-memory `ac8_resume.rs`). (b) `WanLink` injects latency + loss (the lossy/high-RTT harness; transparent-conformance + drop/latency tests) + a DIRECTIONAL round-trip bench (loopback ~1.2 µs vs TCP-loopback ~124 µs/cofre). Still owed: **E4 `bao`** chunk-level resume (#27) and the **FASP-beats-loss-based-TCP** differentiator on a *real* WAN, not loopback (#28). |
