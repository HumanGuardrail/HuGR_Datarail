...
| AC-6 substrate parity | **PROVEN** (harness + 2 substrates) / **PENDING** (real substrates) | `substrate_conformance<S>` passed by both `LoopbackSubstrate` and `ResumableSubstrate` (INV-SUBSTRATE-POLYMORPHIC). The SPEC's shmem/QUIC/S3 **real** substrates are post-v1 (only in-memory substrates exist) |
