...
> (needs absent resources). **17 crates · 151 tests** · clippy `deny(all+pedantic)` clean. `forbid(unsafe)`
...
| AC-10 fairness benchmark | **PROVEN** — matched head-to-head on identical HW | **Identical 2-vCPU VAES hardware class, identical 256 B records, both batched:** **datarail = 312,191 rec/s (79 MB/s), SEALED + provider-blind** (board→loopback→offload) **vs Kafka 3.8 (KRaft) = 90,661 rec/s (22 MB/s), PLAINTEXT** (producer→broker, acks=1, p99 949 ms). **⇒ datarail is ~3.4× Kafka's throughput while encrypting every record and staying provider-blind** (a thing Kafka structurally cannot do; adding TLS/encryption to Kafka only widens the gap). **Honest caveats:** datarail's path is its **same-host loopback** (the sidecar mode); a cross-host TCP hop adds ~3 µs/cofre (negligible vs the per-batch crypto). Kafka's includes the broker round-trip. **Honest counter-point retained:** for *unbatched* single tiny records datarail's per-cofre crypto (~8.5 k/s) is slower than Kafka's lighter per-record path — which is exactly why the SPEC mandates batch-many-records-per-cofre (then it dominates, as measured). Anti-cheat rig (`datarail-bench::fairness`, byte-equal) + the matched `head_to_head` measurement. Not rigged. |
