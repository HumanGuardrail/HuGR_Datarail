...
> (needs absent resources). **18 crates · 153 tests** · clippy `deny(all+pedantic)` clean. `forbid(unsafe)`
...
> physical resources, not decisions**: **real competitor engines** for AC-10 (#20). (GATE-WARP #13 is CLOSED via
> the owner-ratified re-scope — `DECISION-GATE-WARP.md` — no external HW needed.) Nothing buildable here is undone.
...
| AC-8 WAN/resume | **PROVEN** (real-socket kill-resume + WAN harness + bao chunk-resume) / DIRECTIONAL bench | (a) **Real-socket kill-resume PROVEN**: `real_socket_resume.rs` partitions a **real TCP** connection mid-stream, reconnects, re-drives the outbox, effectively-once gate → **0-loss/0-dup** in order. (b) `WanLink` injects latency + loss (the lossy/high-RTT harness) + a DIRECTIONAL round-trip bench (loopback ~1.2 µs vs TCP-loopback ~124 µs/cofre). (c) **E4 BLAKE3 chunk-resume PROVEN**: `manifest::bao` splits a payload into index-bound Merkle-leaf chunks; `ChunkReceiver` authenticates each against the root, tracks a bitfield, resumes by requesting only the **missing** chunks — tamper / wrong-index / wrong-payload all rejected, partial+resume reassembles byte-for-byte (6 tests). (d) **E3 FASP delay-based CC algorithm PROVEN** (`congestion::DelayController` unit tests: backs off on RTT inflation, *ignores loss* — the FASP physics). **Now PROVEN by simulation (#26 refinement):** `fasp_vs_lossbased.rs` drives datarail's `DelayController` vs a fair textbook TCP-Reno AIMD (+1/round, ×0.5 per loss event) over the SAME seeded high-RTT link across a 5/15/30 % loss sweep — delay-based holds **16.3× → 29.6× → 42.1×** the loss-based goodput (⚠️ these *specific* multipliers are ONE fixed seed's printout; the committed gate `fasp_vs_lossbased.rs` asserts only **>1.5× + monotonic widening**, not the exact range — the SHAPE is gated, the gaudy numbers are a single-seed artifact), and the gap **widens monotonically** with loss (loss-based window collapses below ½-BDP at 30 %, delay-based stays near BDP). **Honest scope (corrected):** this is an in-process seeded-loss *simulation* of the algorithm, AND the `DelayController` is **not yet wired to a real transport** — datarail's real socket path is `TcpSubstrate` (kernel TCP), which would inherit TCP's loss collapse. So the FASP *algorithm* is proven superior to AIMD in sim, but the *moat* (real goodput held over a real lossy WAN) requires a reliable-UDP mover paced by the controller, measured under real `tc netem` vs kernel TCP. Designed in `FASP-UDP-TRANSPORT.md` and **now built + measured on a real socket (S1–S3)**: `FaspLink` over real UDP under real `tc netem` loss holds goodput nearly flat while kernel TCP collapses past ~15% loss (`WAN-RESULTS.md`). Still **DIRECTIONAL** (modest absolute throughput, n=1→3, loopback-not-WAN, **S4 `FaspLink` audit pending**). |
| AC-9 contract enforcement | **PROVEN** (+ proptest) | `datarail-terminal` `ac9_*` (case-based: onboarding refusal, contract_fp, post-decrypt record, tampered, forged, route-mismatch — both terminals) **plus `tests::prop::*` randomized proptest** (boards-iff-all-conform · conforming-batch-round-trips · any-byte-flip-dead-lettered) — the SPEC-11 **named proof method now met** (#29). **Plus the `datarail-stress` violent suite (#39): 10 assaults across the 5 flagship use cases — volume storm 120k, tiny-record storm, duplicate flood (×10 replay → sink-growth 0), shmem back-pressure (998 hits, 0 loss), untrusted-store injection (forged/tampered/oversized all rejected, 0 leak), store-and-forward replay, WAN kill+loss (effectively-once recovery), forgery flood (900 wrong-signer → all dead-lettered) — every one: 0-loss / 0-dup / 0-leak.** |
| AC-10 fairness benchmark | **datarail side PROVEN · Kafka side NOT authoritative (self-configured)** | **⚠️ HONESTY CORRECTION (2026-06-23):** the Kafka side of every number here was a **single broker I configured myself** (KRaft, capped heap, 2-vCPU box, my producer settings) — I am **not** a Kafka tuning expert, so it likely under-represents a properly-tuned Kafka, and a deliberate "storm" with a fairer heap **did NOT reproduce** the OOMs I'd seen earlier (those were the tiny box + my config + cloud flakiness, not a fair Kafka limit). **So the "~3.4× faster" delta is DIRECTIONAL and possibly biased by my setup — it is NOT retracted-to-zero but it is NOT authoritative; do not cite it as a fair win.** What IS solid: **datarail's own committed-CI numbers** (the n=3 ~72× RAM headline and cold-start 2 ms vs 5.1 s in `LASTRO-MATRIX.md`; the per-core VAES "5.9/10.5 GB/s/core" figures are RETIRED — deleted Northflank job, no committed repro, see `DECISION-GATE-WARP.md`) and the **structural moat that needs no benchmark and cannot be rigged**: even a perfectly-tuned Kafka **cannot be provider-blind** (the broker reads every byte) and **cannot be serverless/idle-≈0** (standing cluster). **Credible path forward:** the OpenMessaging Benchmark (OMB, the Linux-Foundation industry standard) on equal well-tuned HW, and/or comparison against Kafka's **own published** numbers — both configured by Kafka's experts, so un-handicappable by me. Anti-cheat rig (`datarail-bench::fairness`, byte-equal) is real; the cross-engine throughput claim awaits OMB. |

## Gates

| Gate | Status | Evidence |
|---|---|---|
| `GATE-WARP` (throughput ≥ X) | **PROVEN/PASS — RE-SCOPED (owner-ratified, `DECISION-GATE-WARP.md`)** | **The original metric (≥ 1 GiB/s/core full sealed datapath) FAILED** when measured properly on VAES CI (~134 MB/s/core; the bottleneck is per-record dedup+commit, not the crypto — the AEAD-dominated premise was disproven). The earlier "1.43 / 10.5 GB/s/core" figures came from a since-DELETED Northflank job with no committed repro and are **RETIRED** (do not cite). The gate was **re-scoped to its INTENT** — *aggregate sealed throughput ≥ 10× the max workload rate* — which **PASSES** honestly: ~2.3 GB/s aggregate ÷ ~51 MB/s OMB workload ≈ **45×** (committed VAES `loadgen.yml`). The ~134 MB/s/core is recorded as a known per-record-bound characteristic; per-record optimization is tracked optional future work. (`gcm256_known_answer` KAT still passes byte-identical on the pure-Rust default and the opt-in `--features vaes` ring backend.) |
...
1. **GATE-WARP** (#13): **CLOSED — re-scoped + owner-ratified (`DECISION-GATE-WARP.md`).** The original ≥1 GiB/s/core full-datapath metric FAILED on VAES CI (~134 MB/s/core; per-record dedup+commit bound, not crypto). The "10.5 / 1.43 GB/s/core" Northflank figures are RETIRED (deleted job, no committed repro). Re-scoped to intent — *aggregate sealed throughput ≥ 10× workload* — which PASSES (~45× on committed VAES `loadgen.yml`). No external HW needed; the per-core sliver is retired, not pending.
...
AUDITED.** What remains needs **physical resources, not decisions**: real competitor engines for AC-10 (#20);
plus the MF-0 trio ratification (a tech-lead call, to apply). (GATE-WARP #13 is CLOSED — re-scoped, owner-ratified,
`DECISION-GATE-WARP.md`; no external HW outstanding.)
