...
| **RAM to move data** (measured, same 51 MB/s) | ✅ **~22 MB load · 2-3 MB idle** · ~1.2 cores | ❌ 870 MB load · 275 MB idle · 1.49 cores | ❌ 1,757 MB load · 810 MB idle | ❌ ~256MB–1.5GB | **datarail — ~40× less resident RAM loaded (22 MB vs 870 MB, re-measured), ~90× idle. NOT like-for-like rulers; corrected from an over-claimed 124× by the adversarial audit (`ADVERSARIAL-AUDIT.md`). The disruption axis: footprint, not speed.** |
...
| **Max throughput** (32-core, measured, SEALED) | **engine ~1900 MB/s @1KB / ~2500 @16KB** · full-TCP ~1534 @1KB | **1604 MB/s** (OMB, plaintext) | 296–441 (OMB) | ~38 (pub.) | **datarail's sealed engine WINS ~1.2× @1KB / ~1.6× @16KB** (sealed vs plaintext); full-TCP ~parity. Not 3-4× (that needs encrypted-Kafka). 658→1900 = 2.9× extracted. (`OMB-RESULTS.md` Run 8.) |
