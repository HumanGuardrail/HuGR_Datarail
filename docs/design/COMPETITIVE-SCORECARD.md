...
| **Max throughput** (32-core, measured, SEALED) | **engine ~1900 MB/s @1KB / ~2500 @16KB** · full-TCP ~1534 @1KB | **1604 MB/s** (OMB, plaintext) | 296–441 (OMB) | ~38 (pub.) | datarail's sealed engine ~1.2× @1KB / ~1.6× @16KB (sealed vs plaintext); full-TCP ~parity. ⚠️ **n=1, no variance bars** — these Turbo runs swing 2–3× run-to-run, so treat as DIRECTIONAL until n≥3 (the robust claim is *parity-class*, not a pinned win). Not 3-4× (needs encrypted-Kafka). (`OMB-RESULTS.md` Run 8.) |
