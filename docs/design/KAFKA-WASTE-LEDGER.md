...
| 4 | **Idle cluster waste** — heartbeats/fetch/controller/metadata burn CPU+RAM 24/7 even at zero traffic | Stateless mover, idle ≈ 0 (process exits; scale-to-zero) | **PROVEN** (footprint) / **DIRECTIONAL** (TCO) | 3 MB idle vs Kafka 274 MB (`OMB-RESULTS.md`); **cold-start MEASURED (controlled CI, n=30/5)**: datarail **p50 2 ms** (spread 0) vs Kafka **p50 5.1 s** ⇒ scale-to-zero viable vs always-on (`COLD-START-RESULTS.md`, `cold-start.yml`); TCO model in `EFFICIENCY-TCO.md` |
