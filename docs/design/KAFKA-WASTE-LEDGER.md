...
| 8 | **Cross-region is an afterthought** — MirrorMaker (separate, heavy, TCP, collapses under WAN loss) | FASP-style delay-based CC + sealed: holds goodput under loss; WAN/multi-cloud native | **DIRECTIONAL** | 16–42× vs loss-based TCP in seeded-loss simulation; real lossy-link field run vs MirrorMaker is PENDING |
