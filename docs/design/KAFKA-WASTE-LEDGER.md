...
| 2 | **Page-cache-as-memory** — holds the whole hot working set in RAM (great in 2011; wasteful with NVMe+object-store in 2026) | Tiered-native: small hot tail in-process, durable retained history on disk/S3, streamed on demand | **TIERED PROVEN** (hot-local + cold-remote) | `datarail-replaylog`: a segmented, **retained** (not GC-on-ack) durable log + `replay_from(offset)` streaming reader. Gated: replay a 40 MiB retained volume (~640× the 64 KiB read buffer) with peak buffered RAM **bounded by `REFILL_CHUNK + record` the whole way** — flat, independent of total retained volume; replay-from-arbitrary-offset is exact; **repeatable** (history retained, unlike the WAL's drain+GC — the actual Kafka-replay semantics); reopen-recovery. **Honest scope:** cold tier = local FS today (S3/object-store = the same reader with the GET verb swapped, future); consumer-group offset coordination = future. This is the cold half of the "split the log Kafka fused" architecture, proven. |
...
| 4 | **Idle cluster waste** — heartbeats/fetch/controller/metadata burn CPU+RAM 24/7 even at zero traffic | Stateless mover, idle ≈ 0 (process exits; scale-to-zero) | **PROVEN** (footprint) / **DIRECTIONAL** (TCO) | 3 MB idle vs Kafka 274 MB (`OMB-RESULTS.md`); cold-start: datarail **ms** vs Kafka **seconds** to ready ⇒ scale-to-zero viable vs always-on (`COLD-START-RESULTS.md`; exact figure/multiplier flagged **PENDING-RIGOR** — the regime holds, the number isn't yet pinned); TCO model in `EFFICIENCY-TCO.md` |
| 5 | **Partition count** — fixed up-front, can't shrink, each is files+RAM+leader; high counts hurt controller/recovery/latency | Decouple parallelism from a fixed physical partition count | **PROVEN** (routing) | `datarail-keyrouter`: rendezvous (HRW) routing — **parallelism = consumer count, NO partition config**. Gated: 80k keys over 8 nodes each land within ±10% of an even split (no hot partitions); same key → same node (per-key order). **Honest scope:** the routing *mechanism* is proven; wiring it to the full dispatch/consume loop is future. |
| 6 | **Rebalance hell** — stop-the-world consumer-group reassignment on join/leave | Monotonic rendezvous routing — no global reshuffle on membership change | **PROVEN** (routing) | `datarail-keyrouter` monotonicity gates: **adding** a node moves keys *only to it* (measured: 0 reassignments among existing nodes), churn ~K/(N+1); **removing** a node remaps *only* its own keys (every other key provably untouched). No stop-the-world barrier. **Honest scope:** proven on the routing function; live membership-change orchestration is future. |
...
| 10 | **Config sprawl / operational PhD required + slow cold-start** | The safe thing is the default; minimal knobs; ms cold-start | **DIRECTIONAL** (regime; figure PENDING-RIGOR) | **DIRECTIONAL** (regime, not a pinned number): cold-start to ready — datarail **ms**, Kafka **seconds** (`COLD-START-RESULTS.md`). The ms-vs-s regime (scale-to-zero vs always-on) is robust; the exact 8 ms / 5.5 s / ~690× was a single favorable sample and is flagged **PENDING-RIGOR** (re-run gave ~141 ms / ~22 s). |
