...
| F2 | MED | core/terminal `Gcm256` "key-uniqueness gate" | documented gate not enforced in code | **MITIGATED** by F1 (all algs now use random nonces; with per-cofre keys each key encrypts exactly one message). Residual: `Gcm256` selectable without an explicit gate → tracked as a **hardening TODO** (v1 default is GCM-SIV; doc updated mentally — gate or drop `Gcm256` in a future rev) |
...
| F4 | LOW | terminal `board` | ephemeral X25519 secret not explicitly zeroized (not stored; dropped at frame exit) | **ACCEPTED / hardening-TODO** — needs the `zeroize` dep; forward-secrecy intent holds (never escapes). Tracked. |
| F5 | LOW | terminal `AAD_V1 = &[]` | empty AEAD AAD is safe *only because* the lacre is verified before `aead_open`; safety lives outside the AEAD | **ACCEPTED / hardening-TODO** — bind `aad` to seal-time-final header fields (route‖stream‖seq‖aead_alg‖nonce‖eph_pk) to sidestep the `cofre_id` ordering cycle. Tracked. |
