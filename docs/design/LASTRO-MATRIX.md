...
| **Kafka CONSUME — provider-blind bidirectional drop-in** | datarail-kafka Fetch | `serve_broker` + `DestTerminal::open` + `build_record_batch`(CRC-32C) + `kafka_broker_wire.rs` (full binary) + store unit test | full binary: produce 3 → **fetch back the plaintext**; storage holds **sealed** cofres (provider-blind); ListOffsets → 3 | **BACKED/MEASURED** (2026-06-27) — `kafka-broker` mode; un-seal only at the fetch edge. **Scope (increment 1):** in-memory store, single partition, no groups (durable backing tracked) |
