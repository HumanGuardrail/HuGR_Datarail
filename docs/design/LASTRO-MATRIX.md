...
| **Kafka durable CONSUMER OFFSETS (group commit)** | datarail-kafka groups + offsets | `FindCoordinator`/`OffsetCommit`/`OffsetFetch` (`groups.rs`) + `FileOffsets` + `kafka_groups_wire.rs` (full binary) | full binary: commit offset 7 → **kill broker → restart → OffsetFetch → 7**; cross-group isolated; fsync-before-ack | **BACKED/MEASURED** (2026-06-27) — durable committed offsets survive a broker restart. **Scope (increment 3):** explicit-commit / manual-assignment consumers; automatic group rebalance (`JoinGroup`/`SyncGroup`/`Heartbeat`) tracked (increment 4). Audited `CORE-AUDIT.md` §Kafka-OFFSETS |
