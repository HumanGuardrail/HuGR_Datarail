...
| **exactly-once END-TO-END through the product CLI** | Tier A wiring | `datarail run --sink-postgres` (default Tier A) + `connectors-live.yml` replay assertion | live PG: `datarail run` ×2 (identical replay, fresh process) → **4 rows, not 8**; `--at-least-once` → 8 (as labelled) | **BACKED/MEASURED** (2026-06-26) — the Postgres-resident watermark, not in-memory dedup, enforces once-ness across invocations (distinct cofre_ids, zero re-land) |
