...
fn assert_exactly_once_set(committed: &[Vec<u8>], expected: &[Vec<u8>], scenario: &str) {
    assert_eq!(committed.len(), expected.len(), "{scenario}: sink size != expected (0-loss/0-dup)");
    let mut got: Vec<&Vec<u8>> = committed.iter().collect();
    let mut want: Vec<&Vec<u8>> = expected.iter().collect();
    got.sort();
    want.sort();
    assert_eq!(got, want, "{scenario}: sink multiset != expected (a record was lost or duplicated)");
    ...
    assert_eq!(unique.len(), committed.len(), "{scenario}: a record was committed more than once (0-dup)");
}
...
fn uc5a_loss_storm_redrive_recovers_every_record_exactly_once() {
    ...
    let profile = WanProfile { drop_every: 3, ..WanProfile::default() };
    ...
    while committed.len() < N {
        sessions += 1;
        assert!(sessions <= 64, "loss recovery failed to converge in a sane number of re-drives");
        for c in outbox.iter().filter(|c| !committed.contains(&c.etiqueta.cofre_id)) {
        ...
        while let Some(c) = link.recv().expect("recv") {
            ...
            if already {
                ...
                assert_eq!(disp, Disposition::Delivered, "a first-time cofre must be delivered");
    ...
    assert_exactly_once_set(rig.dest.sink().committed(), &expected, "uc5a wan-loss-storm");
    assert!(rig.dest.dead_letters().is_empty(), "nothing dead-lettered across the lossy link");
    assert_eq!(committed.len(), N, "all N records committed exactly once despite the loss storm");
    assert!(link.dropped() > 0, "the assault must actually drop sends (else it isn't a loss test)");
...
fn uc5b_real_tcp_kill_mid_stream_with_loss_recovers_exactly_once() {
    ...
    assert!(committed.len() <= first_leg, "session 1 committed at most the first leg");
    ...
    assert_exactly_once_set(rig.dest.sink().committed(), &expected, "uc5b wan-kill-mid-stream");
    assert!(rig.dest.dead_letters().is_empty(), "nothing dead-lettered across the partition");
    ...
    while empty_streak < 8 {
        match link.recv().expect("recv") {
            Some(c) => {
                ...
                tally.observe(disp);
                if already {
                    assert_eq!(disp, Disposition::Duplicate, "re-driven committed cofre dedups");
                } else {
